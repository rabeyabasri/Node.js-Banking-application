...
authFunctions.get_login_company = (req, res) => {
  ...
  // res.send({
  //   message: "succesfully signed up. Please login to access account",
  // });
  res.json(res_json);
};
...
authFunctions.get_deposit_withdraw_history = async (req, res) => {
  console.log("user email is"+res.locals.user);
  let e = res.locals.user;
  if (userIsLoggedIn(e)) {
    res_json.method = "GET";
    res_json.url = "user/deposit_withdraw/history";
    let { email } = res.locals.user;
    ...
    let newHistory = [...user["depositHistory"], ...edit_];
...
authFunctions.post_signup_company = async (req, res) => {
  try {
    ...
    res.send("Account created successfully. Log in to access account")
...
authFunctions.post_login_company = async (req, res) => {
  ...
  try {
    ...
    setCookie(user, res, "show_users1");
  } catch (error) {
    handleErrors(error, res, true);
  }
};
...
authFunctions.post_user_info_update = async (req, res) => {
  ...
  res_json.method = "POST";
  res_json.url = "user/info/update";
  res_json.update = "true";

  delete res_json.depositHistory;
  delete res_json.withdrawHistory;

  delete res_json.user;
  delete res_json.history;
  res.json({ update: res_update });
};
authFunctions.post_transferMoney = async (req, res) => {
  ...
  if (recipientType === `user`) {
    ...
    res_json.url = "money/transfer/user";
  } else if(recipientType === `company`) {
    message = await Company.transferMoneyTo(email, amount, receiverID);
    res_json.url = "money/transfer/company";
  }
  else{
    res.send("please enter the right recipient type");
    return;
  }

  res_json.money_transfered = "true";
